...
class Page extends BaseController
{
    function __construct()
    {
        ...
        $this->halaman_controller = "page"; //utk konfigurasi internal
        ...
        if($this->request->getMethod()=='post'){
            ...
            if(!$this->validate($aturan)) {
                ...
                if ($aksi != false) {
                    ...
                    $set_halaman_tren= $this->request->getVar('set_halaman_tren');
                    $set_halaman_tips= $this->request->getVar('set_halaman_tips');
                    $set_halaman_agenda= $this->request->getVar('set_halaman_agenda');
                    ...
                    if ($dataGet !== null && isset($dataGet['konfigurasi_value'])) {
                    ...
                    /** set halaman tren.*/
                    $konfigurasi_name = "set_halaman_tren";
                    $dataGet = konfigurasi_get($konfigurasi_name);
                    if ($dataGet !== null && isset($dataGet['konfigurasi_value'])) {
                        if ($set_halaman_tren == '1') {
                            $page_id_tren = $page_id;
                        }
                        if ($dataGet['konfigurasi_value'] == $page_id && $set_halaman_tren != '1') {
                            $page_id_tren = '';
                        }
                        $dataSet = [
                            'konfigurasi_value' => $page_id_tren
                        ];
                        konfigurasi_set($konfigurasi_name, $dataSet);
                    }

                    /** set halaman tips.*/
                    $konfigurasi_name = "set_halaman_tips";
                    $dataGet = konfigurasi_get($konfigurasi_name);
                    if ($dataGet !== null && isset($dataGet['konfigurasi_value'])) {
                        if ($set_halaman_tips == '1') {
                            $page_id_tips = $page_id;
                        }
                        if ($dataGet['konfigurasi_value'] == $page_id && $set_halaman_tips != '1') {
                            $page_id_tips = '';
                        }
                        $dataSet = [
                            'konfigurasi_value' => $page_id_tips
                        ];
                        konfigurasi_set($konfigurasi_name, $dataSet);
                    }
                    ...
                    $dataGet = konfigurasi_get($konfigurasi_name);
                    if ($dataGet !== null && isset($dataGet['konfigurasi_value'])) {
                        if ($set_halaman_agenda == '1') {
                            $page_id_agenda = $page_id;
                        }
                        if ($dataGet['konfigurasi_value'] == $page_id && $set_halaman_agenda != '1') {
                            $page_id_agenda = '';
                        }
                        $dataSet = [
                            'konfigurasi_value' => $page_id_agenda
                        ];
                        konfigurasi_set($konfigurasi_name, $dataSet);
                    }
    ...
    function editartikel($post_id)
    {
        ...
        $dataGet = konfigurasi_get('set_halaman_tren');
        if ($dataGet !== null && isset($dataGet['konfigurasi_value'])) {
            if ($dataGet['konfigurasi_value'] == $post_id) {
                $data['set_halaman_tren'] = 1;
            }
        }
        $dataGet = konfigurasi_get('set_halaman_tips');
        if ($dataGet !== null && isset($dataGet['konfigurasi_value'])) {
            if ($dataGet['konfigurasi_value'] == $post_id) {
                $data['set_halaman_tips'] = 1;
            }
        }
        $dataGet = konfigurasi_get('set_halaman_agenda');
        if ($dataGet !== null && isset($dataGet['konfigurasi_value'])) {
            if ($dataGet['konfigurasi_value'] == $post_id) {
                $data['set_halaman_agenda'] = 1;
            }
        }
